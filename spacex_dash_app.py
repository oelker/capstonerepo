...
def get_pie(value):
    ...
    if value == 'ALL':
        fig = px.pie(filtered_df, values='class', title='Total Success Launches By Site', names='Launch Site')
        return fig
    else:
        ...
        fig = px.pie(filtered_df, values='class count', title=title, names='class')
        return fig
# TASK 4:
...
def get_scatter(value1, value2):
    ...
    if value1=='ALL':
        ...
        fig= px.scatter(filtered_df_2, x='Payload Mass (kg)', y='class', color='Booster Version Category', title="Correlation between Payload and Success for site {value1]")
        return fig
# Run the app
